...
\newcommand{\executionflowfigure}{%
  \IfFileExists{execution_flow.png}{%
    \includegraphics[width=0.86\textwidth,height=0.5\textheight,keepaspectratio]{execution_flow.png}%
  }{%
    \IfFileExists{execution_flow.jpg}{%
      \includegraphics[width=0.86\textwidth,height=0.5\textheight,keepaspectratio]{execution_flow.jpg}%
    }{%
      \IfFileExists{code_execution_flow.png}{%
        \includegraphics[width=0.86\textwidth,height=0.5\textheight,keepaspectratio]{code_execution_flow.png}%
      }{%
        \IfFileExists{code_execution_flow.jpg}{%
          \includegraphics[width=0.86\textwidth,height=0.5\textheight,keepaspectratio]{code_execution_flow.jpg}%
        }{%
...
\texttt{return} & Return value from a flow & return & \texttt{return x + 1} \\
\texttt{break} & Exit current loop & break & \texttt{break;} \\
\texttt{continue} & Skip to next iteration & continue & \texttt{continue;} \\
\texttt{extends} & Inheritance declaration & extends & \texttt{symphony C extends P} \\
\bottomrule
\end{tabularx}
\end{table}
...
\texttt{!=} & Inequality & \texttt{!=} & \texttt{x != y} \\
...
Operator precedence from highest to lowest: unary \texttt{!} and \texttt{-}, then \texttt{*} and \texttt{/}, then \texttt{+} and \texttt{-}, then comparisons (\texttt{< <= > >= == !=}), then \texttt{\&\&}, then \texttt{||}.
...
The \texttt{score} loop supports optional initialization and step expressions; any or all three parts may be omitted. If the condition is omitted, the loop runs until terminated with \texttt{break}. A \texttt{continue} statement transfers control to the step expression before the next iteration.
...
At program level, the language supports top-level flows and uses \texttt{main} as the normal entry point when present. If no \texttt{main} flow is defined, execution begins at the first top-level flow in the program. The \texttt{take(...)} parameter part may also be omitted for flows with no parameters.
...
Formatted output through \texttt{play} supports \texttt{\%d}, \texttt{\%f}, \texttt{\%s}, and \texttt{\%b}. Constants declared with \texttt{fixed} cannot be reassigned later and support only scalar types (\texttt{int}, \texttt{float}, \texttt{bool}, \texttt{string}).
...
\texttt{obj.field} / \texttt{obj.method()} & Field access and instance method calls \\
\texttt{TypeName.method(args)} & Static-style method call on a named type \\
...
\begin{itemize}
\item single-line comments using \texttt{//} and multiline comments using \texttt{/* ... */}
...
\Needspace{10\baselineskip}
\subsection*{C++ Emit Mode}

The compiler includes a dedicated output mode activated with \texttt{--emit=cpp}. In this mode the AST is translated into a readable C++-style representation without executing the program. Two presentation styles are available: \texttt{--emit-style=cpp} uses \texttt{auto} and \texttt{emit(...);}, while \texttt{--emit-style=pseudo} uses \texttt{var} and \texttt{print(...);}. The emitter prepends a keyword legend mapping ORCHESTRA constructs to their C++ equivalents. Comparison chains such as \texttt{a < b < c} are rewritten as \texttt{((a < b) \&\& (b < c))} to match C++ evaluation semantics.
